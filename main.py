...
class AddressSimilaritySystem:
    ...
    def preprocess_addresses(self, df: pd.DataFrame,
        ...
        print(f"预处理完成: {len(df)} 条地址")
        print(f"  - 标准化成功: {self.processing_stats['normalized_addresses']}")
        print(f"  - 地理编码成功: {self.processing_stats['geocoded_addresses']}")
    ...
    def find_similar_candidates(self, df: pd.DataFrame) -> List[Tuple[int, int]]:
        ...
        print(f"   找到 {len(text_candidates)} 个文本相似候选对")

        # 方法2: 基于行政区划的候选对
        print("2. 基于行政区划查找候选对...")
        admin_candidates = self._find_admin_candidates(address_data)
        print(f"   找到 {len(admin_candidates)} 个行政区划候选对")

        # 方法3: 基于地理空间的候选对
        print("3. 基于地理空间查找候选对...")
        spatial_candidates = self._find_spatial_candidates(address_data)
        print(f"   找到 {len(spatial_candidates)} 个空间候选对")
        ...
        print(f"候选对总数: {len(all_candidates):,}")
    ...
    def _find_spatial_candidates(self, address_data: Dict[int, Dict[str, Any]]) -> List[Tuple[int, int]]:
        ...
        print(f"   有坐标的地址: {len(addresses_with_coords)}")
    ...
    def calculate_similarities(self, df: pd.DataFrame,
        ...
        # 批量计算相似度
        print(f"计算 {len(candidate_pairs):,} 个候选对的相似度...")
        ...
        print(f"相似度计算完成:")
        print(f"  - 候选对总数: {len(candidate_pairs):,}")
    ...
    def cluster_addresses(self, df: pd.DataFrame,
        ...
        print(f"聚类完成:")
        print(f"  - 聚类数量: {quality['n_clusters']}")
        print(f"  - 类内平均相似度: {quality['avg_similarity_within']:.3f}")
        print(f"  - 类间平均相似度: {quality['avg_similarity_between']:.3f}")
        print(f"  - 轮廓系数: {quality['silhouette_score']:.3f}")
    ...
    def generate_results(self, df: pd.DataFrame,
        ...
        print(f"1. 聚类结果保存到: {cluster_file}")
        ...
        print(f"2. 聚类摘要保存到: {summary_file}")
        ...
        print(f"3. 相似度对保存到: {similarity_file}")
    ...
    def print_summary(self, df: pd.DataFrame, quality: Dict[str, Any]):
        ...
        print("=" * 60)
        print(f"📊 统计摘要:")
        print(f"   总地址数: {self.processing_stats['total_addresses']:,}")
        print(f"   标准化成功: {self.processing_stats['normalized_addresses']:,}")
        print(f"   地理编码成功: {self.processing_stats['geocoded_addresses']:,}")
        print(f"   候选对数量: {self.processing_stats['candidate_pairs']:,}")
        print(f"   相似对数量: {self.processing_stats['similar_pairs']:,}")
        print(f"   聚类数量: {self.processing_stats['clusters_found']:,}")
        print(f"   处理时间: {duration:.2f}秒")
        print(f"   处理速度: {self.processing_stats['total_addresses'] / duration:.1f} 地址/秒")
        print(f"\n🎯 聚类质量:")
        print(f"   轮廓系数: {quality.get('silhouette_score', 0):.3f}")
        print(f"   类内平均相似度: {quality.get('avg_similarity_within', 0):.3f}")
        print(f"   类间平均相似度: {quality.get('avg_similarity_between', 0):.3f}")
        print(f"   聚类大小分布:")
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
      ...
      使用示例:
      # 测试运行（1万条数据）
      python main.py --input data/sample.csv --output test_run --sample 10000
    
      # 全量运行
      python main.py --input data/addresses.csv --output full_run
